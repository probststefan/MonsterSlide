...
public class Hud {
	...
	public void generateSlideOverview() {
		...
		// Pixmap pixmap = new Pixmap((int) GameScreen.settings.SLIDE_DIMENSION
		// + offset,
		// (int) GameScreen.settings.SLIDE_DIMENSION + offset, Format.RGBA8888);
		//
		// Array<ISlidePart> slideParts =
		// gameScreen.world.getSlide().getSlideParts();
		// Array<Vector3> vertices =
		// slideParts.first().getInterpolatedVertices();
		...
		// pixmap.drawLine((int) first.x + (offset / 2), (int) first.z + (offset
		// / 2),
		...
		// pixmap.fillCircle((int) v.x + (offset / 2), (int) v.z + (offset / 2),
		// 3);
		// }
		//
		// Vector3 tmp = slideParts.first().getControlPoints().first();
		// pixmap.setColor(Color.YELLOW);
		// pixmap.fillCircle((int) tmp.x + (offset / 2), (int) tmp.z + (offset /
		// 2), 3);
		//
		...
		// pixmap.fillCircle((int) tmp.x + (offset / 2), (int) tmp.z + (offset /
		// 2), 3);
		// mapOverview.setDrawable(new TextureRegionDrawable(new
