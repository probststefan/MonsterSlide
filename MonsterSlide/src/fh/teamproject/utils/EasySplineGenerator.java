...
public class EasySplineGenerator {
	...
	private void generateSplines() {
		...
		Vector3[] points = new Vector3[] { new Vector3(-10.0f, 0.0f, -5.0f),
				new Vector3(25.0f, 0.0f, 25.0f), new Vector3(25.0f, 0.0f, 25.0f),
				new Vector3(100.0f, -this.slope, 100.0f) };
		...
		for (int i = 0; i < this.numOfSplines; i++) {
			...
			points[1] = new Vector3(points[3].x - points[2].x + points[3].x, points[3].y
					- points[2].y + points[3].y, points[3].z - points[2].z + points[3].z);

			// Endpunkt der Spline
			// points[3] = new Vector3(MathUtils.random(0.0f, this.width), 0.0f,
			// MathUtils.random(0.0f, this.width));

			points[3] = new Vector3(points[3].x + 100, points[3].y - this.slope,
					points[3].z + 100);

			points[2] = new Vector3(MathUtils.random(points[0].x, points[3].x),
					points[3].y + this.slope / 2.0f, MathUtils.random(points[0].z,
							points[3].z));
		}
	}
}